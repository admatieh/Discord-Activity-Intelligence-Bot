...
module.exports = {
    ...
    async execute(message, args, context) {
        ...
        if (typeof voiceChannelId === 'object' && voiceChannelId !== null) voiceChannelId = voiceChannelId.id;
        else if (voiceChannelId) voiceChannelId = String(voiceChannelId).replace(/[<#>]/g, '');

        let guildId = context?.guild?.id || message?.guild?.id;
        if (typeof guildId === 'object' && guildId !== null) guildId = guildId.id;

        if (typeof voiceChannelId === 'object' && voiceChannelId !== null) return '❌ voiceChannelId must be a string ID.';
        ...
        if (!rawTimeInput) return '❌ --at (e.g. "tomorrow 2:30 PM") or --in (e.g. "30m") is required.';
