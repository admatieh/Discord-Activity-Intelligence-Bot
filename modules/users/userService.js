...
async function syncAllMembers(guild) {
    try {
        ...
        logger.error(`userService.syncAllMembers error: ${error.message}`, { error: error.message });
    }
}
...
function syncMember(member) {
    try {
        ...
        logger.error(`userService.syncMember error: ${error.message}`, { memberId: member.id });
    }
}
